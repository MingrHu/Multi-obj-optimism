...
- **测试前必须复核 Python 环境**：不得因当前终端未激活、`PATH` 未包含 Python，或某一个
  启动入口执行失败，就判断仓库 `.venv` 不可用。应先按仓库安装脚本的使用方式激活环境，
  再检查解释器实际路径、Python 版本和目标依赖导入，并实际执行一个最小测试。
- 宣布测试环境不可用前，必须至少使用两种独立入口反复验证，并再次执行目标测试；需要区分
  “当前终端未激活”“单个启动入口异常”“缺少某项依赖”和“整个虚拟环境不可用”，只报告
  已由命令输出直接证明的问题，不根据单次失败扩大结论。
- marker：
