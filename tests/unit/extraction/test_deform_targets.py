...
def test_extract_max_load_last_frame():
    ...
    result = _extractMaxLoad([frame], obj="topdie", inprogress=False)
...
def test_extract_max_load_inprogress_takes_max():
    ...
    result = _extractMaxLoad([f1, f2, f3], obj="topdie", inprogress=True)
    assert result == "250.00"


def test_extract_grain_stdv():
    # USRELM 1 3 -> 随后 3 行，arr[3] 为晶粒尺寸
    frame = [
        "USRELM 1 3 x x",
        "1 a b 10.0",
        "2 a b 20.0",
    ...
    result = _extractGrainStdv([frame], obj="workpiece", inprogress=False)
    import statistics
...
def test_extract_max_stress():
    ...
    result = _extractMaxStress([frame], obj="1", inprogress=False)
