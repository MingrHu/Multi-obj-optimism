...
`biz_params` 可以省略或传空对象，此时使用所选模型族的全部默认值。模型参数由
`mobo.surrogate.hyperparameters` 统一合并和校验，最终训练与 K 折交叉验证使用同一组
完整有效参数；未知字段、类型错误、越界值和不一致的参数组合会在训练前返回失败响应。
外部客户端应优先通过 HTTP 元数据接口读取参数定义，不应复制内部默认值。
