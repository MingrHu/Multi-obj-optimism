...
从 ``AutoScript/auto_script_test.py`` 搬入的手写 demo（非 pytest），演示采样→初始化→
运行→提取的完整调用链。仅将 import 改为 ``mobo.*``、``PROJECT_DIR`` 取自
:mod:`mobo.common.paths`，并去除 ``sys.path.append``。
...
from mobo.automation.service import (CreateSmpGenTask,InitExecutionTask,RunExtractData,
