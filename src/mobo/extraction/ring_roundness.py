...
本模块仅包含可复用的纯函数与数据结构（原 ``Geo/deform_ring_roundness.py``
的算法部分，byte-for-byte 保留），命令行入口见 :mod:`mobo.cli.ring_roundness`。
另提供 :func:`extract_ring_roundness` 作为原子能力层的适配器。
"""
...
def extract_ring_roundness(
    ...
    *,
    ...
    """原子能力适配器：从 KEY 文件提取指定圈（内/外）的圆度。

    复用本模块现有纯函数组合完成完整流程，供 :mod:`mobo.extraction.registry`
    以 ``key_file`` 约定注册；不改动任何底层算法函数体。
    ...
    :param z_tol: 截面判定容差（None 时按模型尺度自动确定）
    :param merge_tol: 端点合并容差（None 时按模型尺度自动确定）
    :return: 圆度（优先最小区域圆 MZC，缺 scipy 时回退最小二乘圆 LSC）
    """
    if which not in ("inner", "outer"):
        raise ValueError(f"which 只能为 'inner' 或 'outer'，收到：{which!r}")
    ...
    merge_tol = default_merge_tol if merge_tol is None else merge_tol

    boundary_faces = extract_boundary_faces(mesh.elements)
    segments = extract_section_segments(
        mesh, boundary_faces, plane_z=plane_z, z_tol=z_tol, merge_tol=merge_tol
    )
    ...
    for component in components:
        ...
        loop, _ = order_component_as_loop(component, point_map)
        if len(loop) < 3:
            continue
        area = abs(polygon_signed_area(loop))
        component_infos.append({"perimeter": perimeter, "loop": loop, "area": area})

    if len(component_infos) < 2:
        raise ValueError(
            "未获得两个有效的闭合截面轮廓，无法区分内外圈。"
            f"当前有效连通分量数：{len(component_infos)}。"
        )

    # 取周长最大的两个主轮廓，再按包围面积区分内外圈。
    ...
    loop = selected[0]["loop"] if which == "inner" else selected[1]["loop"]

    result = analyze_profile(which, loop, samples)
    circle = result.minimum_zone or result.least_squares
    return float(circle.roundness)
