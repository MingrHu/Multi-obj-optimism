...
从 ``AutoScript/utils.py`` 搬入的 KEY/DB 处理、求解调度与采样函数。这些函数通过
子进程驱动 Windows 平台的 ``DEF_PRE_64.exe`` / ``DEF_ARM_CTL.COM``，仅能在装有
DEFORM 的 Windows 环境真实运行；在其他平台仅用于单元测试（打桩）。

与原实现的差异仅限工程化处理：
- import 来源改为 ``mobo.*``；
- Windows 反斜杠路径拼接改为跨平台的 :func:`os.path.join`。
...
def ProcessEXTRACT_DB_FILE(res_db_file:list[str],
    ...
    for i,dbfile in enumerate(res_db_file):
        ...
        # 创建目录
        res_save_path = os.path.join(res_key_path, str(i))
    ...
    with open(os.path.join(res_txt_path, f"{dt_string}_result.txt"), "w", encoding="utf-8") as f:
