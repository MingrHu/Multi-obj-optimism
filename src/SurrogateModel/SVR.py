...
def stdv_train_SVR(train_times = 20):
    ...
    # 训练结束后，保存最佳模型
    save_best_model("stdv",best_stdv_model,best_stdv_r2,best_fact_stdv,best_stdv_pred,scalers,"SVR")
    return (mean_stdv_r2 ,mean_stdv_nmae,max_time,min_time)  

def load_train_SVR(train_times = 20): 
    ...
    # 训练结束后，保存最佳模型
    save_best_model("load",best_load_model,best_load_r2,best_fact_load,best_load_pred,scalers,"SVR")
    return  (mean_load_r2,mean_load_nmae,max_time,min_time)   


def RunSVR(train_times):

    t1 = Time()
    t1.start()
    mean_stdv_r2 ,mean_stdv_nmae,max_stdv_time,min_stdv_time = stdv_train_SVR(train_times)
    t1.stop()
    sum_stdv_time = t1.get_duration("ms")

    t2 = Time()
    t2.start()
    mean_load_r2,mean_load_nmae,max_load_time,min_load_time = load_train_SVR(train_times)
    t2.stop()
